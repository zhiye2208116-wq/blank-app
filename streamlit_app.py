    ...
    **注意事項：**
    ...
    if submitted:
        if not time_slots:
            ...
            conflict = df[(df["設備"] == equipment) & (df["日期"] == str(date)) & (df["時段"].isin(time_slots)) & (df["狀態"].isin(["待審核", "借用中"]))]
            if not conflict.empty:
                st.error("⚠️ 部分選擇的時段已被預約！")
            else:
                order_id = str(uuid.uuid4())[:8]
                apply_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                new_records = pd.DataFrame([[order_id, name, department, equipment, str(date), slot, purpose, "待審核", apply_time, ""] for slot in time_slots],
    ...
    if password == "SNSPJ1103":
        ...
        if pending.empty:
            ...
            for idx, row in pending.iterrows():
                st.markdown(
                    f"訂單編號: {row['訂單編號']} | 姓名: {row['姓名']} | 部門: {row['部門']} | 設備: {row['設備']} | 日期: {row['日期']} | 時段: {row['時段']} | 目的: {row['借用目的']} | 申請時間: {row['申請時間']}"
                )
